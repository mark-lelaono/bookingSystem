...
class APIService {
  ...
  async getRooms(): Promise<RoomsResponse> {
    ...
    if (response.ok) {
      ...
      // Backend returns array directly or paginated results
      return {
  ...
  async getMyBookings(): Promise<BookingsResponse> {
    ...
    if (response.ok) {
      const data = await response.json();
      // Backend returns { upcoming_bookings, recent_bookings, statistics }
      const allBookings = [...(data.upcoming_bookings || []), ...(data.recent_bookings || [])];
      return {
        results: allBookings
      };
    }
