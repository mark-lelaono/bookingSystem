...
const LoginPage: React.FC = () => {
  ...
  const handleForgotPassword = async (e: React.FormEvent) => {
    ...
    try {
      // Note: Backend password reset endpoint needs to be implemented
      setSuccess("Password reset functionality will be available soon. Please contact admin.");
      setTimeout(() => {
        setMode("login");
        setForgotEmail("");
      }, 3000);
  ...
  const handleVerifyOTP = async (e: React.FormEvent) => {
    ...
    try {
      await apiService.verifyEmail(verifyEmail, otpCode);
